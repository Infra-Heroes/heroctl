...
func TestSSHCommand_Store(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.Contains(r.URL.Path, "/api/instances/123/ssh") {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		...
		// Read framed payload from client
		payload, err := readClientFrame(conn)
		if err != nil {
			return
		}

		if payload == "hello stdin" {
			// Write unframed payload wrapped as server WS binary frame
			_, _ = conn.Write(buildServerFrame(2, []byte("hello stdout")))
		}
	}))
	defer ts.Close()
	...
	c := client.New("http://localhost:12345", "auth.test.com", "client-id", tok)
	deps := &Deps{
		Token:  tok,
		Client: c,
	}
	...
	cmd := sshCmd(deps)
	cmd.SetArgs([]string{"123", "--store", "--store-url", ts.URL})
